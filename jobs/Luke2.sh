...
cd $TMPDIR
mkdir "$HOME/Scratch/job_files/Luke2"
...
"{'train_frac':0.2, 'offsets':[0], 'depth':14, 'thresholds':1, 'max_features':'sqrt'}"

cp -r $TMPDIR "$HOME/Scratch/job_files/Luke2/$SGE_TASK_ID (`date +"%d-%m-%y %T"`)"
