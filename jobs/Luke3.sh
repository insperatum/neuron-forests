...
cd $TMPDIR
mkdir "$HOME/Scratch/job_files/Luke3"
...
"{'train_frac':0.2, 'offsets':[-2, 0, 2], 'depth':14, 'thresholds':1, 'max_features':'sqrt'}"

cp -r $TMPDIR "$HOME/Scratch/job_files/Luke3/$SGE_TASK_ID (`date +"%d-%m-%y %T"`)"
